...
uint8 CAN_CalculateXOR8(uint8* buffer_pu8, uint8 dlc, uint8 bitpos)
{
	uint8 data = 0;
	uint8 chksum = 0;
	uint8 chksumbyte = bitpos/8;
	int i;
...
uint8 CAN_CalculateXOR4(uint8* buffer_pu8, uint8 dlc, uint8 bitpos)
{
	uint8 data = 0;
	uint8 chksum = 0;
	uint8 chksumbyte = bitpos/8;
	...
	for (i = 0; i < dlc; i++)
	{
		...
		{
			data = (bitpos & 0x07) ? (buffer_pu8[i] & 0x0F) : (buffer_pu8[i] & 0xF0);
		}
		chksum = chksum ^ data;
	}

	chksum = ((chksum & 0xF0)>>4) ^ (chksum & 0x0F);
	return (chksum);
}


static const uint8 CAN_CRC8Table[256] =
{
